...
pub fn query_test() {
  assert <<"Q":utf8, 13:int-size(32), "SELECT 1":utf8, 0>>
    == encode.query("SELECT 1")
}

pub fn password_test() {
  assert <<"p":utf8, 24:int-size(32), "supersecurepassword":utf8, 0>>
...
pub fn describe_test() {
  assert <<"D", 6:int-size(32), "S":utf8, "":utf8, 0>>
    == encode.describe(encode.Statement, "")
}

pub fn flush_test() {
  assert <<"H":utf8, 4:int-size(32)>> == encode.flush()
}

pub fn sync_test() {
  assert <<"S":utf8, 4:int-size(32)>> == encode.sync()
}
